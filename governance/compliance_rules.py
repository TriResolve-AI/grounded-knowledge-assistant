...
UPDATED: Captures full telemetry required for the Microsoft Responsible AI Toolbox.
"""
...
class ComplianceEngine:
    ...
    def _check_cr001(self, citation_count: int) -> RuleResult:
        ...
        return RuleResult(
            ...
            message=("PASS: Response includes citations." if passed else "FAIL: Response has no citations. Cannot surface ungrounded answers."),
        )
    ...
    def _check_cr002(self, trust_score: int) -> RuleResult:
        ...
        return RuleResult(
            ...
            severity="BLOCK", passed=passed, business_function="grounding",
            message=(f"PASS: Trust score {trust_score} meets minimum ({MINIMUM_TRUST_SCORE})." if passed else f"FAIL: Trust score {trust_score} is below minimum threshold ({MINIMUM_TRUST_SCORE})."),
        )
    ...
    def _check_cr003(self, risk_flag: str) -> RuleResult:
        ...
        return RuleResult(
            ...
            message=("PASS: No high-risk language detected." if passed else "WARN: Response contains high-risk terms. Human review recommended before acting."),
        )
    ...
    def _check_cr004(self, citation_strength: str, response_text: str) -> RuleResult:
        ...
        return RuleResult(
            ...
            severity="BLOCK", passed=passed, business_function="grounding",
            message=("PASS: Response is grounded in retrieved document chunks." if passed else "FAIL: Response was generated with no retrieved source chunks. Cannot proceed."),
        )
    ...
    def _check_cr005(self, confidence_level: str) -> RuleResult:
        ...
        return RuleResult(
            ...
            message=("PASS: Confidence level is acceptable." if passed else "WARN: Confidence is LOW. A disclaimer will be shown to the user."),
        )
    ...
    def _check_cr006(self, citation_count: int) -> RuleResult:
        ...
        return RuleResult(
            ...
            severity="WARN", passed=passed, business_function="grounding",
            message=(f"PASS: {citation_count} sources referenced — good diversity." if passed else "WARN: Only 1 source referenced. Consider expanding the knowledge base."),
        )
    ...
    def _check_cr007(self, response_text: str) -> RuleResult:
        ...
        return RuleResult(
            ...
            message=("PASS: Response length is within expected range." if passed else f"WARN: Response is suspiciously short ({len(response_text)} chars). Possible retrieval failure."),
        )
    ...
    def _check_cr008(self, risk_score_result: dict) -> RuleResult:
        ...
        return RuleResult(
            ...
            message=(f"PASS: Citations are from current document versions (freshness={freshness_flag})." if passed else f"WARN: {stale_count} citation(s) are from outdated document version(s). Verify currency before acting on this response."),
        )
    ...
    def _check_cr009(self, confidence_level: str, risk_flag: str) -> RuleResult:
        ...
        return RuleResult(
            ...
            message=("PASS: Confidence/risk combination does not require escalation." if not needs_escalation else "FAIL: LOW confidence + HIGH risk detected. This response must be reviewed by a qualified human before release."),
        )

    def _determine_decision_status(self, blocked: list, warned: list, confidence_level: str, risk_flag: str) -> str:
    ...
    def _build_disclaimer(self, blocked: list, warned: list, confidence_level: str, risk_flag: str) -> Optional[str]:
        if blocked:
            blocked_ids = [r.rule_id for r in blocked]
            if "CR-009" in blocked_ids:
                return "🔄 This response requires human review before it can be released. It has been flagged for escalation."
            return "⛔ This response did not pass compliance checks and cannot be displayed. Please refine your query or contact your administrator."
        parts = []
        if confidence_level == "LOW":
            parts.append("ℹ️ This answer has low confidence. Verify with source documents before acting.")
        if risk_flag == "HIGH":
            parts.append("⚠️ This response contains regulatory or legal language. Please consult a qualified professional before making decisions.")
        if risk_flag == "MEDIUM":
            parts.append("ℹ️ Some parts of this answer may be subject to interpretation. Review the cited sources for full context.")
    ...
    def _build_audit_entry(
        ...
        MODIFIED: Captures full texts required for Responsible AI Toolbox metrics.
        """
        return {
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "query_preview": query[:100] + ("..." if len(query) > 100 else ""),
            
            # --- RAI TOOLBOX ADDITIONS ---
            "full_query": query, 
...
def check_compliance(
    ...
    MODIFIED: Added raw_citations and user_role to feed the RAI Toolbox audit entry.
    """
